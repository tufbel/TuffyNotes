...
```shell
tar -xvf FileName.tar -C path       # 解包
tar -cvf FileName.tar DirName  # 将DirName和其下所有文件（夹）打包
tar -zcvf  FileName.tar.gz file1 file2 file3 # 将所有文件压缩成tar.gz 
# 注意，tar默认使用的是相对路径，不建议使用绝对路径，因为压缩时会吧路径也压缩
tar -zxvf  FileName.tar.gz -C path # 将FileName.tar.gz解压到path
```
...
使用journalctl命令可以查看后台任务的运行日志

```shell
journalctl [-n LINE_NUM] [-f] [-u SERVER_NAME] 
```

- `-n`：指定查看条数，`LINE_NUM`为数字
- `-f`：追踪日志，自动刷新
- `-u`：指定日志后台服务名称，使用`systemctl`启动的服务可以使用`-u`参数指定名称查看。

> 此命令可以结合`grep`命令与进程PID过滤使用。
